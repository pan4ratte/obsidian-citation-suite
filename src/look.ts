import { CitationUnderline, ZoterikSettings } from "src/types";
...
const CUSTOM_COLOR_VAR = "--zoterik-citation-custom-color";
...
const ALL_CLASSES = [
	...
	...UNDERLINES.map((underline) => `zoterik-citation-underline-${underline}`),
];
...
export function lookClasses(
	...
	if (isAccentColor(settings.citationColor)) {
		classes.push("zoterik-citation-color-accent");
	} else if (isCustomColor(settings.citationColor)) {
		classes.push("zoterik-citation-color-custom");
	}
	if (UNDERLINES.includes(settings.citationUnderline)) {
		classes.push(`zoterik-citation-underline-${settings.citationUnderline}`);
	}
	// Strictly `true`: a hand-edited "false" string is not a yes.
	if (settings.citationBold === true) {
		classes.push("zoterik-citation-bold");
	}
	if (settings.citationItalic === true) {
		classes.push("zoterik-citation-italic");
	}
	return classes;
}

/** Puts the look on one window's body, replacing whatever was there. */
export function applyLook(body: HTMLElement, settings: ZoterikSettings): void {
	clearLook(body);
